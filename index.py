"""
================================================================================
🖼️ OPTIMIZED IMAGE CATEGORIZATION PIPELINE
================================================================================

Author: Sadaf (PhD Candidate - Cryptography)
Purpose: Categorize millions of images with AI models, store in SQL Server

FLOW:
1. Fetch images from AWS S3 Private Bucket (batch processing)
2. Compress to web-friendly size (100-250KB)
3. Upload compressed to AWS S3 Public Bucket
4. AI Categorization (SigLIP for category, BLIP-2 for captions/tags)
5. Store metadata in Microsoft SQL Server

ARCHITECTURE:
- AsyncIO for I/O operations (AWS download/upload)
- ThreadPool for CPU operations (image compression)
- Sequential GPU batching for AI models (thread-safe)
...
class Config:
    ...
    # S3 Buckets
    S3_PRIVATE_BUCKET: str = "your-private-bucket-name"  # Original images (private)
    S3_PUBLIC_BUCKET: str = "your-public-bucket-name"    # Compressed images (public)
    S3_PRIVATE_PREFIX: str = "images/"                   # Folder prefix in private bucket
    S3_PUBLIC_PREFIX: str = "thumbnails/"                # Folder prefix in public bucket
    
    # ----- SQL Server Configuration -----
    SQL_SERVER: str = "your-server.database.windows.net"
    SQL_DATABASE: str = "ImageCatalog"
    ...
    MAX_IMAGES_TO_PROCESS: int = 100    # Limit for testing (set to None for all)
    ...
    def insert_batch(self, metadata_list: List[ImageMetadata]) -> int:
        ...
        for meta in metadata_list:
            try:
                ...
                self.logger.warning(f"⚠️ Duplicate image skipped: {meta.image_id[:16]}...")
            except Exception as e:
...
class S3Manager:
    ...
    async def upload_compressed(
        ...
        return f"s3://{self.config.S3_PUBLIC_BUCKET}/{compressed_key}"
    ...
    def compress_single(
        ...
        try:
            ...
            metadata = {
                ...
                's3_original_path': f"s3://{self.config.S3_PRIVATE_BUCKET}/{original_key}",
            }
...
class AICategorizationEngine:
    ...
    SUB_CATEGORIES = {
        "Nature & Landscapes": [
            "Mountains", "Beaches", "Forests", "Deserts", "Lakes", 
            "Sunsets", "Flowers", "Seasons", "Weather", "Gardens"
        ],
        "People & Portraits": [
            "Families", "Children", "Professionals", "Groups", "Seniors",
            "Couples", "Friends", "Celebrations", "Lifestyle", "Emotions"
        ],
        "Business & Corporate": [
            "Office", "Meetings", "Teamwork", "Finance", "Marketing",
            "Leadership", "Startup", "Remote Work", "Presentations", "Analytics"
        ],
        "Technology & Digital": [
            "Computers", "Smartphones", "AI & Robotics", "Software", "Networks",
            "Cybersecurity", "Cloud Computing", "Gaming", "VR/AR", "Electronics"
        ],
        "Food & Cuisine": [
            "Healthy Food", "Fast Food", "Desserts", "Beverages", "Cooking",
            "Restaurants", "Ingredients", "International Cuisine", "Baking", "Organic"
        ],
        "Travel & Tourism": [
            "Airports", "Hotels", "Landmarks", "Adventure", "Beach Vacation",
            "City Tours", "Road Trips", "Cruises", "Camping", "Cultural"
        ],
        "Medical & Healthcare": [
            "Hospitals", "Doctors", "Medical Equipment", "Research", "Wellness",
            "Pharmacy", "Dental", "Mental Health", "Fitness", "Diagnostics"
        ],
        "Animals & Wildlife": [
            "Pets", "Wild Animals", "Birds", "Marine Life", "Insects",
            "Farm Animals", "Exotic", "Endangered", "Baby Animals", "Safari"
        ],
        "Architecture & Buildings": [
            "Modern", "Historic", "Interiors", "Exteriors", "Skyscrapers",
            "Residential", "Commercial", "Religious", "Bridges", "Urban"
        ],
        "Art & Abstract": [
            "Paintings", "Digital Art", "Patterns", "Textures", "Colors",
            "Minimalist", "3D Art", "Illustrations", "Graffiti", "Sculptures"
        ],
        "Sports & Fitness": [
            "Team Sports", "Individual Sports", "Gym", "Outdoor Sports", "Water Sports",
            "Winter Sports", "Extreme Sports", "Athletes", "Equipment", "Training"
        ],
        "Fashion & Beauty": [
            "Clothing", "Accessories", "Makeup", "Hairstyles", "Jewelry",
            "Models", "Street Style", "Luxury", "Casual", "Seasonal"
        ],
        "Education & Learning": [
            "Schools", "Universities", "Libraries", "E-Learning", "Students",
            "Teachers", "Books", "Science", "Research", "Classrooms"
        ],
        "Entertainment & Media": [
            "Movies", "Music", "TV Shows", "Gaming", "Celebrities",
            "Concerts", "Theater", "Streaming", "Social Media", "Events"
        ],
        "Transportation & Vehicles": [
            "Cars", "Motorcycles", "Planes", "Trains", "Ships",
            "Bicycles", "Public Transit", "Electric Vehicles", "Classic Cars", "Trucks"
        ]
    }
    ...
    def __init__(self, config: Config, logger: logging.Logger):
        ...
        self.logger.info(f"🖥️ AI Engine initialized on: {self.device}")
        if self.device == "cuda":
    ...
    def load_models(self):
        ...
        self.logger.info("🔹 Loading BLIP-2 model for captioning...")
    ...
    def process_batch(
        ...
        for img_bytes, _ in compressed_images:
            try:
                ...
                self.logger.warning(f"⚠️ Failed to open image: {e}")
    ...
    def unload_models(self):
        ...
        self.logger.info("🗑️ AI models unloaded from memory")
...
class CheckpointManager:
    ...
    def clear(self):
        ...
        if os.path.exists(self.checkpoint_file):
            ...
            self.logger.info("🗑️ Checkpoint file cleared")
...
class ImageCategorizationPipeline:
    ...
    async def run(self):
        ...
        try:
            ...
            self.logger.info("\n☁️ Step 3: Listing images from S3...")
            images_to_process, _ = await self.s3_manager.list_images(
                max_images=self.config.MAX_IMAGES_TO_PROCESS
            )
            
            if not images_to_process:
                self.logger.warning("⚠️ No images found to process!")
                return
            
            self.logger.info(f"   Found {len(images_to_process)} images to process")
            
            # Step 4: Process in batches
            self.logger.info("\n⚙️ Step 4: Processing images in batches...")
            ...
            for batch_idx in range(total_batches):
                ...
                if not filtered:
                    self.logger.info("   ⏭️ All images in batch already processed, skipping...")
                    continue
...
async def main():
    ...
    config = Config(
        ...
        S3_PRIVATE_BUCKET="your-private-images-bucket",
        S3_PUBLIC_BUCKET="your-public-thumbnails-bucket",
        
        # SQL Server Settings - UPDATE THESE
        SQL_SERVER="your-server.database.windows.net",
        SQL_DATABASE="ImageCatalog",
        
        # Processing Settings
        MAX_IMAGES_TO_PROCESS=100,  # Set to None for all images
...
if __name__ == "__main__":
    print("""
    ╔══════════════════════════════════════════════════════════════╗
    ║     🖼️  OPTIMIZED IMAGE CATEGORIZATION PIPELINE  🖼️          ║
    ╠══════════════════════════════════════════════════════════════╣
    ║  Features:                                                   ║
